...
class EarlyStopping:
    ...
    def __init__(self, save_path, idx, patience=50, verbose=False, delta=0):
        """
        Args:
    ...
    def __call__(self, score, affiliation, rpa_score, model):
        ...
        if self.best_score is None:
            ...
            self.best_aff_f1 = 2 * (affiliation["precision"] * affiliation["recall"]) / (affiliation["precision"] +
                                     affiliation["recall"])
            self.best_rpa_score = rpa_score
            self.save_checkpoint(score, model)
        elif (2 * (affiliation["precision"] * affiliation["recall"]) / (affiliation["precision"] +
              affiliation["recall"])) < self.best_aff_f1 + self.delta:
            self.counter += 1
